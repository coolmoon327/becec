...
class DP:
    def __init__(self, inputs, tours):
        ...
        # 这个位置的 c_slots, p_slots 的顺序需要颠倒一下
    ...
    def improve_score(self):
        ...
        # 最开始的顺序是否需要记住
        tours = self.tours
        batch, task_num = tours.shape
        workload = self._workload
        alpha = self._penalty_factor

        # 从前往后排序task的执行顺序, 也就是说这里把排序的任务也做了
        tours_seq = alpha.argsort()
        tours = tours_seq  # 完全新的任务顺序
        workload = np.array([workload[i][tours_seq[i]] for i in range(
            workload.shape[0])])
        alpha = np.array([alpha[i][tours_seq[i]] for i in range(alpha.shape[
                                                                    0])])
        trace = np.full((batch, self._task_size, self._slots_size),
                        fill_value=0,
                        dtype=np.int)
        # 从前往后使用slot资源(slot的workload大于0)
        for i in range(batch):
            cur_task = 0
            cur_slot = 0
            while cur_task < self._task_size and cur_slot < self._slots_size:
                workload_tmp = workload[0][cur_task]
                while workload_tmp > 0:
                    trace[0][cur_task][cur_slot] = min(workload_tmp,
                                                       self._c_slots[0][
                                                           cur_slot])
                    workload_tmp -= trace[0][cur_task][cur_slot]
                    # 这里可以表示当前的时隙资源用完了吗?然后cur_slot才会+1?
                    cur_slot += 1 if trace[0][cur_task][cur_slot] == \
                                     self._c_slots[0][cur_slot] else 0
                    cur_task += 1 if workload_tmp == 0 else 0
        # 根据 trace 来计算总的 u 和总的 cost
        # 总的 cost 用 向量乘法就可以了
        ...
        # 总的 u 需要知道最后一个正数是什么 > 0 arg, 只用一个for循环就可以找到所有的
    ...
    def score(self):
        ...
        for b in range(batch):
            ...
            if (-np.max(dp[b][self._task_size - 1]) == INF):
                ...
                # 需要验证这个位置的 tours 是不是全部都被设置为 -1 了, 如果没有, 后面会出问题
...
class DP2:
    def __init__(self, inputs, tours):
        ...
        c_slots = c_slots[::-1]  # 翻转
        p_slots = p_slots[::-1]  # 翻转
