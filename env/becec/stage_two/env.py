...
class Env_tsp():
	...
	def get_batch_task_nodes(self, n_samples, seed = None):
		...
		if seed is not None:
			torch.manual_seed(seed)
		device = torch.device('cuda:0' if torch.cuda.is_available() else 'cpu')
		task_workload = 10 * torch.rand((n_samples, self.city_t, 1), device = device) + 5
		task_penalty_factor = torch.rand((n_samples, self.city_t, 1), device = device) # fixme: task 参数名需要改变
	...
	def get_batch_env_nodes(self, n_samples, seed = None):
		...
		device = torch.device('cuda:0' if torch.cuda.is_available() else 'cpu')
		env_computation_power = 10 * torch.rand((n_samples, self.slots, 1), device = device) + 5
		env_price = 9 * torch.rand((n_samples, self.slots, 1), device = device) + 1 # fixme: env 参数名需要改变
		env_info = torch.cat((env_computation_power, env_price), 2)
		# reshape env to (batch, slots * 2)
	...
	def seq_score(self, inputs, tours): # fixme: 排序只需要做一次就行，内部使用顺序直接切片加上堆的做法
		...
		dp = np.full((batch, task_size, slots_size), fill_value = -INF, dtype = np.float32)
		real_worker = np.full((batch, slots_size, slots_size), fill_value=0, dtype=np.int) # (batch, slots_size, slots_size) i： 多少批次 j：以 j 为 last_t 方案 k：具体方案的具体实施方法
		total_utility = np.full((batch, task_size, slots_size), fill_value=0, dtype=np.float32) # 和 dp 模块等大小,记录整体的 utility
		# worker_trace = np.full((batch, task_size, slots_size), fill_value=0, dtype=np.int) # 完整记录每个时间点上每个任务分配了多少资源

		workload,  penalty_factor = np.split(task_data, 2, axis=2) # workload then penalty_factor
				...
				if i == 0 : # 特殊处理第一个任务
					...
					while j >= 0: # 从后往前遍历
						if (c_sum_slots[b][j] >= workload[b][cur_task_index]):
							worker = np.full((slots_size, ), fill_value=0, dtype=np.int) # 记录可行的一个方案, 针对 i 任务的可行方案
							cost = 0
							utility = 0
							last_t = 0
							rank = p_slots[b][:j+1].argsort() # 按照rank从左到右使用时隙
							rank = iter(rank)
							...
							while (workload_temp > 0):
								...
								workload_temp = workload_temp - c_slots[b][p_min_slot]
								cost = cost + c_slots[b][p_min_slot] * p_slots[b][p_min_slot]
								worker[p_min_slot] = c_slots[b][p_min_slot] # 某一个方案中使用了某一个时隙的资源，有多少
							utility = -last_t * penalty_factor[b][cur_task_index]
							if p_min_slot == last_t and workload_temp < 0: # 最后使用的时隙是 last_t 并且有资源的剩余
								worker[last_t] = worker[last_t] + workload_temp # 第一个任务的 last_t
								cost = cost + workload_temp * p_slots[b][last_t]
								workload_temp = 0
								if utility - cost > dp[b][0][last_t]: # 最后的时隙无剩余完成任务
									real_worker[b][last_t] = copy.deepcopy(worker) # 刚开始完全复制 是没有问题的, 而且一定不需要考虑 last_t 重复的问题
									dp[b][0][last_t] = utility - cost
									total_utility[b][0][last_t] = utility # 第一个任务, 先记录所有的 utility
									remain_cpu[last_t] = -workload_temp
							if p_min_slot == -1: # 初始任务负载为0: 如果进入这里 默认的是 last_t 就是0
								remain_cpu[last_t] = c_slots[b][last_t] # 如果说使用的成本很高呢？一定要牢记使用量必定是 0
							...
							if p_min_slot != -1: # 当前任务的工作量大于 0 
								cost = cost + workload_temp * p_slots[b][p_min_slot] # 有一部分的 c_slots[b][p_min_slot] 是没有用完的
							if utility - cost > dp[b][0][last_t]: # 完成任务的最后一个时隙不是 last_t 是在中间的某一个位置
								if p_min_slot != -1:
									worker[p_min_slot] = worker[p_min_slot] + workload_temp
								real_worker[b][last_t] = copy.deepcopy(worker) # 刚开始完全复制 是没有问题的, 而且一定不需要考虑 last_t 重复的问题
								dp[b][0][last_t] = utility - cost
								total_utility[b][0][last_t] = utility # 第一个任务, 先记录所有的 utility
								remain_cpu[last_t] = 0
					...
					slots_posible_stratage = np.array([], dtype=np.int) # 记录可行方案的时刻
					for j in range(slots_size): # 清除右边比左边小的值
						if dp[b][0][j] <= largest_num_left:
							dp[b][0][j] = -INF
							total_utility[b][0][j] = 0 # 不具备可行方案
							real_worker[b][j] = [0 for _ in range(slots_size)] # 如果说没有可行方案, 将last_t对应的方案全部置零
						else:
							largest_num_left = dp[b][0][j]
							slots_posible_stratage = np.append(slots_posible_stratage, j) # 对应的位置才需要查询是否有remain的cpu资源, 下面的对应方法一定是将第一层的worker 做过清除
				else: # i > 0
					largest_num_left = -INF # 记录左侧的最大值
					remain_temp = [0 for _ in range(slots_size)] # 从此处开始使用 remain_temp，作用是记录完成这个任务过程中有哪些时隙是最后使用的并且有可以使用的剩余资源
					worker_temp = np.full((batch, slots_size, slots_size), fill_value=0, dtype=np.int) # 记录本轮任务完成时的完整使用轨迹
					for j in slots_posible_stratage: # 可能的左端点
						right = slots_size
						while (right > j): # 每进入一次， remain_cpu 都应该和上一个任务结束时的完全一致
							cost = 0
							utility = 0
							if remain_cpu[j] > 0: # 上面的所有任务在j完成，并且在j时隙还有剩余的资源可以使用
								if (c_sum_slots[b][right - 1] - c_sum_slots[b][j] + remain_cpu[j]) >= workload[b][cur_task_index]:
									worker = np.full((slots_size, ), fill_value=0, dtype=np.int)
									last_t = 0
									...
									workload_temp = workload[b][cur_task_index]
									p_min_slot = -1
									remain_cpu_term = copy.deepcopy(remain_cpu) # remain_cpu_term上一个任务结束后有多少可以使用的资源，可以在不同的起始端点重复使用remain_cpu_term，对它进行修改相当于一个标志，用来辅助修改cpu_temp
									if workload_temp <= 0: # 跳过workload为0的情况
										if dp[b][i - 1][j] > dp[b][i][j]: # 涉及对 remain_temp[j] 的重复赋值，只有当 dp 的取值更加时，对 remain_temp[j] 重复赋值
											dp[b][i][j] = dp[b][i - 1][j]
											total_utility[b][i][j] = total_utility[b][i - 1][j] - j * penalty_factor[b][cur_task_index] # 整体的utility会增加一个在  j 时刻结束的 分量
											remain_temp[j] = remain_cpu[j] # 最后一个时隙完成了任务，并且还有剩余的资源
											worker_temp[b][j] = real_worker[b][j] # 对最后一个时隙的重复
											# worker = copy.deepcopy(real_worker[b][j]) # 即使仍然需要赋值，需要做的事情是 real_worker[b][j] = real_worker[b][j]
									else: # 如果说 workload 非 0
										workload_temp -= remain_cpu_term[j] # 首先使用一部分 cpu 资源, 但是在while循环中会重复使用 remain_cpu 的资源, 因此在排序的时候应该和remain_cpu为0的方式相同考虑
										cost = cost + remain_cpu_term[j] * p_slots[b][j] # 首先得默认 remain_cpu 使用完毕
										worker[j] += remain_cpu_term[j] # 默认使用 j 时隙的所有剩余 cpu 资源, 首先使用了 remain 的资源，但是问题是以62结尾的应该没有剩余的资源了，为什么还要继续使用呢
										if workload_temp < 0: # 依旧无法使用完 remain_cpu[j] ，j 依旧是最后的一个时隙
											remain_cpu_term[j] = -workload_temp # 给本轮使用
											worker[j] += workload_temp # 剩余 cpu 资源多的退回去
											last_t = j
											utility = -last_t * penalty_factor[b][cur_task_index]
											cost = cost + workload_temp * p_slots[b][j]
											if ((utility - cost) + dp[b][i - 1][j]) >= dp[b][i][last_t]: # 新的分配方案更优
												dp[b][i][last_t] = ((utility - cost) + dp[b][i - 1][j])
												total_utility[b][i][last_t] = total_utility[b][i - 1][j] + utility
												remain_temp[j] = -workload_temp # remain_temp[j] 是否会重复赋值，应该选取其中的哪一个？应该选取dp数值更大的那一个
												worker_temp[b][last_t] = real_worker[b][j] + worker # 上次结束使用的时隙是 j , 现在还是在使用 last_t的时隙
											workload_temp = 0 # 资源反还完毕，刚好完成任务	
										while (workload_temp > 0): # 使用完了上个任务剩余时隙的资源,接着还需要找
											p_min_slot = next(rank, -1)
											last_t = max(last_t, p_min_slot)
											workload_temp = workload_temp - c_slots[b][p_min_slot]
											cost = cost + c_slots[b][p_min_slot] * p_slots[b][p_min_slot]
											worker[p_min_slot] = c_slots[b][p_min_slot] # 记录在某一个时隙使用了多少资源
										utility = -last_t * penalty_factor[b][cur_task_index]
										if p_min_slot == last_t and workload_temp < 0: # 任务完成了，并且最后的一个时隙有剩余cpu的情况
											worker[last_t] = worker[last_t] + workload_temp # 最后一个时隙多使用的资源不能记录在worker
											cost = cost + workload_temp * p_slots[b][last_t] # 发现有一部分资源没有使用完
											workload_temp = 0 # 完成该任务
											if ((utility - cost) + dp[b][i - 1][j]) > dp[b][i][last_t]: # 只在这种情况下进行重新赋值是否正确呢？ 我的答案应该是正确的
												worker_temp[b][last_t] = worker + real_worker[b][j] # 上一次的 j 还有残留 不用考虑和下面的 if 重叠的状态
												remain_temp[last_t] = -workload_temp # 这种情况下需要对remain_temp 进行更新
												dp[b][i][last_t] = ((utility - cost) + dp[b][i - 1][j])
												total_utility[b][i][last_t] = total_utility[b][i - 1][j] + utility
										if p_min_slot != -1: # 当前任务的工作量大于 0 
											cost = cost + workload_temp * p_slots[b][p_min_slot]
										if ((utility - cost) + dp[b][i - 1][j]) > dp[b][i][last_t]: # 这种状态下说明最后完成的 slot 没有剩余的资源，仅仅是对 dp 进行一次更新
											if p_min_slot != -1:
												worker[p_min_slot] += workload_temp # 多使用了一部分的 c_slots 资源
											worker_temp[b][last_t] = worker + real_worker[b][j] # 上一次的 j 还有残留
											dp[b][i][last_t] = ((utility - cost) + dp[b][i - 1][j])
											total_utility[b][i][last_t] = total_utility[b][i - 1][j] + utility
								...
								if (c_sum_slots[b][right - 1] - c_sum_slots[b][j]) >= workload[b][cur_task_index]: # 从时隙 j + 1开始能满足工作负载要求
									worker = np.full((slots_size, ), fill_value=0, dtype=np.int)
									last_t = 0
									rank = p_slots[b][j + 1 : right].argsort()
									rank = rank + j + 1
									...
									if workload_temp <= 0: # 当前的任务需要的资源量是 0 , 最后一个完成的时隙是 j, 且完成了任务
										if dp[b][i - 1][j] > dp[b][i][j]:
											dp[b][i][j] = dp[b][i - 1][j]
											total_utility[b][i][j] = total_utility[b][i - 1][j] - j * penalty_factor[b][cur_task_index]
											remain_temp[j] = remain_cpu[j]
											worker_temp[b][j] = real_worker[b][j]
									else: # 当前任务的资源需求量大于0
										p_min_slot = -1
										while (workload_temp > 0):
											p_min_slot = next(rank, -1)
											last_t = max(last_t, p_min_slot)
											workload_temp = workload_temp - c_slots[b][p_min_slot]
											worker[p_min_slot] = c_slots[b][p_min_slot] # 记录下某一个时隙使用了具体多少资源
											cost = cost + c_slots[b][p_min_slot] * p_slots[b][p_min_slot]
										utility = -last_t * penalty_factor[b][cur_task_index]
										if p_min_slot == last_t and workload_temp < 0: # 在最后的一个时隙完成了任务，并且需要更新 remain_temp 和 dp
											worker[last_t] = worker[last_t] + workload_temp # 实际环境少使用的资源需要吐出来
											cost = cost + workload_temp * p_slots[b][p_min_slot] # 少使用的花费
											workload_temp = 0
											if ((utility - cost) + dp[b][i - 1][j]) >= dp[b][i][last_t]:
												remain_temp[last_t] = -workload_temp
												dp[b][i][last_t] = ((utility - cost) + dp[b][i - 1][j])
												total_utility[b][i][last_t] = total_utility[b][i - 1][j] + utility
												worker_temp[b][last_t] = (worker + real_worker[b][j])
										if p_min_slot != -1: # 当前任务的工作量大于 0 
											cost = cost + workload_temp * p_slots[b][p_min_slot]
										if ((utility - cost) + dp[b][i - 1][j]) >= dp[b][i][last_t]: # 做的事情完成任务的时隙不是最后的时隙，在这种情况下，只需要更新 dp, remain_cpu 不需要更新, workload_temp 同样可能是小于 0 的
											if p_min_slot != -1:
												worker[p_min_slot] += workload_temp
											dp[b][i][last_t] = ((utility - cost) + dp[b][i - 1][j]) # 上一个任务在 j 时隙已经完成了
											total_utility[b][i][last_t] = total_utility[b][i - 1][j] + utility
											worker_temp[b][last_t] = (worker + real_worker[b][j]) # 全新的数, j 是上一个任务完成的时隙
											remain_temp[last_t] = 0 # 需要清空最后一个时隙的资源
									right = last_t
								else:
									right -= 1
					slots_posible_stratage = np.array([], dtype=np.int) # 记录可行方案的时刻
					for j in range(slots_size): # 清除右边比左边小的值
						if dp[b][i][j] <= largest_num_left:
							...
							slots_posible_stratage = np.append(slots_posible_stratage, j)
					remain_cpu = copy.deepcopy(remain_temp) # 此处更新位置没有错
					real_worker = copy.deepcopy(worker_temp) # 此处还需要更新 real_worker 的数据
		...
		for b in range(batch):
			...
			max_index = np.argmax(dp[b][task_size - 1]) # 无可行方案时如何记录, 无可行方案时, d[b] 返回的是 
			actor_source[b] = copy.deepcopy(real_worker[b][max_index]) # 如果出现没有可行方案怎么办？是否应该返回无法执行
			u[b] = total_utility[b][task_size - 1][max_index]
			if (-np.max(dp[b][task_size - 1]) == INF):
				d[b] = 500
				u[b] = 0 # 如果没有可行方案, 则返回的utility为0
		trace = np.full((batch, task_size, slots_size), fill_value = 0, dtype = np.int)
		for b in range(batch):
			...
			for i in range(len(tours[b])): # 第 i 个任务
				...
				while workload_temp > 0: # 当前任务没有完成
					if actor_source[b][slot] > 0: # 有时隙资源 如果说没有可行方案，还需要管 trace 的计算吗？如何判断没有可行方案？ 根据打分大小吗？
...
def random_train_task(batch, city_t, seed = None):
	...
	task_penalty_factor = torch.rand((batch, city_t, 1), device = device) # fixme: task 参数名需要改变
	task_info = torch.cat((task_workload, task_penalty_factor), 2)
