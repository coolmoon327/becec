...
class Stage_Two_Pointer:
    ...
    def execute(self):
        ...
        for i in range(self._env.config['M']):
            ...
            tasks = copy.copy(
                tasks)  # 二阶段进行了更改, 因此不再允许 tasks 队列随着分配被删除, 需要拿到一个独立于 _env 的队列
            tasks_num = len(tasks)
            if tasks_num == 0:
                continue
            ...
            a = self.test.u[0] - self.test.score[0]
            print(f"贪心输出效果: {self.test.u[0] - self.test.score[0]}")
            self.test.dp_search()
            b = self.test.u[0] - self.test.score[0]
            print(f"dp输出的效果 : {self.test.u[0] - self.test.score[0]}")
            ...
            for index in range(len(tours)):
                ...
                if ((
                            penalty_mode == 3 or penalty_mode == 4) and not self.test_mode) or \
                        self._env.config['force_ignore_bad_tasks']:
