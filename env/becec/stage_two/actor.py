...
class PtrNet1(nn.Module):
    def __init__(self, cfg, config):
        super().__init__()
        self.Embedding = nn.Linear(2, cfg.embed,
                                   bias=False)  # fixme: 输入维度可能就不是 2 了
        self.Env_Embedding = nn.Linear(config['delta_t'] * 2, cfg.embed, bias=
        False)  #
        # 刚刚好环境的维度也是 2, 但是融合了slots 和 p c 数据
        self.Encoder = nn.LSTM(input_size=cfg.embed, hidden_size=cfg.hidden,
                               batch_first=True)
        self.Decoder = nn.LSTM(input_size=cfg.embed, hidden_size=cfg.hidden,
                               batch_first=True)
        if torch.cuda.is_available():
            ...
            self.Vec2 = nn.Parameter(torch.FloatTensor(
                cfg.embed))  # what does it mean by vec, feature vector ?
        ...
        self.city_selecter = {'greedy': Greedy(),
                              'sampling': Categorical()}.get(cfg.decode_type,
                                                             None)

    def _initialize_weights(self, init_min=-0.08, init_max=0.08):
        for param in self.parameters():
            nn.init.uniform_(param.data, init_min,
                             init_max)  # make all parameters in ptr1 uniform
    ...
    def forward(self, x, device):
        ...
        batch, slots_features, _ = env_data.size()
        task_data = task_data.to(device)

        embed_enc_inputs = self.Embedding(task_data)

        env_data = env_data.view(batch, -1)
        env_embed_enc_inputs = self.Env_Embedding(
            env_data)  # todo: 做一次扩展，env_embed..的大小应该与data_embed..一致
        env_embed_enc_inputs = env_embed_enc_inputs.unsqueeze(1).repeat(1,
                                                                        city_t,
                                                                        1)
        embed_enc_inputs = embed_enc_inputs.add(env_embed_enc_inputs)
        embed = embed_enc_inputs.size(2)
        mask = torch.zeros((batch, city_t), device=device)
        enc_h, (h, c) = self.Encoder(embed_enc_inputs,
                                     None)  # fixme: 进入这一步之前应该要将embed_enc_inputs变为两个embedding的相加
        ref = enc_h
        pi_list, log_ps = [], []  # pi_list: pointer list ?
        dec_input = self.dec_input.unsqueeze(0).repeat(batch, 1).unsqueeze(
            1).to(device)
    ...
    def glimpse(self, query, ref, mask, inf=1e8):
        """	-ref about torch.bmm, torch.matmul and so on
            ...
            (batch, city_t)
        """
        u1 = self.W_q(query).unsqueeze(-1).repeat(1, 1, ref.size(
            1))  # u1: (batch, 128, city_t)
        u2 = self.W_ref(ref.permute(0, 2, 1))  # u2: (batch, 128, city_t)
        V = self.Vec.unsqueeze(0).unsqueeze(0).repeat(ref.size(0), 1,
                                                      1)  # why Vec accur data, from where
        ...
        a = torch.softmax(u / self.softmax_T,
                          dim=1)  # a contain the probability of visiting some cities.
    ...
    def pointer(self, query, ref, mask, inf=1e8):
        """	Args:
            ...
            (batch, city_t)
        """
        u1 = self.W_q2(query).unsqueeze(-1).repeat(1, 1, ref.size(
            1))  # u1: (batch, 128, city_t)
...
def test_embedding(x, device):
    ...
    Embedding = nn.Linear(slots_features, embedding_env,
                          bias=False)  # 做线性映射，映射指定为最后一个维度
