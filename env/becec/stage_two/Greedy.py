...
class Greedy(object):
    def __init__(self, inputs, tours):
        ...
        '''
        sequence_key = np.array([p_slots[i] + 0.01 * i for i in range(len(p_slots)) \
                                 for _ in range(batch)])
        slot_use_seq = np.array([np.argsort(sequence_key[i]) for i in range(batch)])
        final_slot = np.full((batch,), fill_value=-1)
        last_slot = np.full((1,), fill_value=-1)
